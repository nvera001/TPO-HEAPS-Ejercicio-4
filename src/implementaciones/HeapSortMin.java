...
public class HeapSortMin implements HeapSortTDA{
    ...
    public void insertar(int valor) { //Se insertan valores en el heap.
        if (cantidad == capacidad) {  //En caso de que el heap (ESTATICO), haya llegado a su capacidad.
            ...
            heap[cantidad] = valor; //Inserta valor en el nodo.
    ...
    public void hundir(int[] arr, int cantidad, int i) { //Para ordenar el arreglo.
        ...
        if (izquierdo < cantidad && arr[izquierdo] < arr[mayor]) //Hijo izquierdo menor que la cantidad, hijo izquierdo menor que el padre.
            mayor = izquierdo; //Izquierdo pasa a ser el mayor.

        if (derecho < cantidad && arr[derecho] < arr[mayor]) //Hijo derecho menor que la cantidad, hijo derecho menor que el padre.
